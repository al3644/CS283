...
int open_db(char *dbFile, bool should_truncate){
    ...
    //open the file if read and write exists 
    //create it if not 
...
int get_student(int fd, int id, student_t *s){
	...
	//check
    	if (fd < 0 || s == NULL || id < MIN_STD_ID || id > MAX_STD_ID) {
        	return ERR_DB_OP;
	}
	
	int index = id % MAX_STD_ID;
    	int offset = index * STUDENT_RECORD_SIZE;
	...
	//error if does not match 
    	if (read(fd, &temp, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        	return ERR_DB_FILE;
    	}

// if student has id and is not deleted, copy and return
    	if (temp.id == id && temp.id != DELETED_STUDENT_ID) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
	...
	int offset = index * STUDENT_RECORD_SIZE;
    	...
    	if (read(fd, &student, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        	memcpy(&student, &EMPTY_STUDENT_RECORD, sizeof(student_t));
    	}
...
int del_student(int fd, int id){
    	...
    	// check
    	if (id < MIN_STD_ID || id > MAX_STD_ID || id == DELETED_STUDENT_ID) {
        	printf(M_STD_NOT_FND_MSG, id);
        	return ERR_DB_OP; 
    	}

	// use get_student to find student 
    	// else, error 
    	student_t student;
	...
	int index = id % MAX_STD_ID;
    	...
    	// reset to offset and check
    	if (lseek(fd, offset, SEEK_SET) == -1) {
        	printf(M_ERR_DB_WRITE);
        	return ERR_DB_FILE;  
    	}

    	// write an empty student record to delete student
...
int count_db_records(int fd){
	...
	// check
    	...
    	while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
        	// if record is not empty, increment
        	if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            		count++;
        	}
    	}
...
int print_db(int fd){
	...
	//check
    	...
    	// check if header has been printed
    	...
    	ssize_t bytes_read;
    	while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
        	// Check if the record is not empty and print
        	if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            		if (!header_printed) {
