...
int start_server(char *ifaces, int port, int is_threaded){
    ...
    if (is_threaded){
        ...
        printf("Server in multi-threaded mode.\n");
    }
    else{
        printf("Server in single-threaded mode.\n");
    }
...
int boot_server(char *ifaces, int port){
    ...
    int enable=1;
    setsockopt(svr_socket, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));
    //may need check to see if it fails
...
int exec_client_requests(int cli_socket) {
    ...
    extern Built_In_Cmds match_command(const char *input);
    ...
    while(1) {
        ...
        if (rc != OK){
            if (rc == ERR_TOO_MANY_COMMANDS) {
                ...
                send_message_string(cli_socket, "error: command or arguments too big\n");
            } else if (rc == ERR_MEMORY) {
                send_message_string(cli_socket, "error: memory allocation error\n");
            } else {
                send_message_string(cli_socket, "unknown error occurred\n");
            }
        ...
        Built_In_Cmds built_in_cmd = match_command(cmd_list.commands[0].argv[0]);
        if (built_in_cmd != BI_NOT_BI) {
            exec_built_in_cmd(&cmd_list.commands[0]);
            send_message_string(cli_socket, "built-in command executed\n");
        ...
        if (cmd_rc < 0) {
            send_message_string(cli_socket, "execution failed\n");
        }
        ...
        //maybe need to add send_message_strings for each thing that happens
...
int send_message_string(int cli_socket, char *buff){
    ...
    while (total_sent < buff_len) {
        bytes_sent = send(cli_socket, buff + total_sent, buff_len - total_sent, 0);
        if (bytes_sent < 0) {
            return ERR_RDSH_COMMUNICATION;
        }
